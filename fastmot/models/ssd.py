...
class SSD:
    ...
    def build_engine(cls, trt_logger, batch_size, calib_dataset=Path.home() / 'VOCdevkit' / 'VOC2007' / 'JPEGImages'):
        ...
        # convert TensorFlow ssd_graph into UFF
...
class SSDMobileNetV1(SSD):
    ...
    def add_plugin(cls, ssd_graph):
        ...
        # Create a new ssd_graph by collapsing namespaces
        ssd_graph.collapse_namespaces(namespace_plugin_map)
        # Remove the outputs, so we just have a single output node (NMS).
        # If remove_exclusive_dependencies is True, the whole ssd_graph will be removed!
        ...
        ssd_graph.find_nodes_by_name("Input")[0].input.remove("image_tensor:0")
...
class SSDMobileNetV2(SSD):
    ...
    # ENGINE_PATH = Path(__file__).parent / 'frozen_inference_graph.trt'
    # MODEL_PATH = Path(__file__).parent / 'frozen_inference_graph.pb'
...
class SSDInceptionV2(SSD):
    ...
    def add_plugin(cls, ssd_graph):
        ...
        # Create TRT plugin nodes to replace unsupported ops in Tensorflow ssd_graph
        Input = gs.create_plugin_node(
            name="Input",
            op="Placeholder",
            dtype=tf.float32,
